...
'freund':'amigo','freunde':'amigos','freunden':'amigos','freundin':'amiga / novia','frau':'mujer / señora','mann':'hombre','kind':'niño','eltern':'padres','mutter':'madre','vater':'padre','bruder':'hermano','schwester':'hermana','verheiratet':'casado','ledig':'soltero','geburtstag':'cumpleaños','hochzeit':'boda',
...
'termin':'cita','wochenende':'fin de semana','woche':'semana','wochen':'semanas','abend':'tarde / noche','mittag':'mediodía','nacht':'noche','stunde':'hora','stunden':'horas','minute':'minuto','minuten':'minutos','gestern':'ayer','morgen':'mañana','übermorgen':'pasado mañana','vorgestern':'anteayer','monat':'mes','monate':'meses','datum':'fecha','kalender':'calendario',
'preis':'precio','preise':'precios','kosten':'costar / costos','kaufen':'comprar','verkaufen':'vender','geschäft':'tienda / negocio','laden':'tienda / cargar','markt':'mercado','supermarkt':'supermercado','angebot':'oferta','rabatt':'descuento','günstig':'económico','teuer':'caro','kostenlos':'gratis','konto':'cuenta bancaria','bank':'banco','karte':'tarjeta / mapa','bar':'en efectivo','sparen':'ahorrar','zahlen':'pagar','wechseln':'cambiar',
...
function glossParts(esRaw:string){const cleaned=clean(esRaw).replace(/\s+/g,' ').trim();if(!cleaned)return[];return cleaned.split(/\s*[;/]\s*/).map(x=>x.trim()).filter(Boolean)}
function glossScore(gloss:string){
  ...
  const common=words.filter(w=>COMMON_ES.has(w.replace(/[¿?¡!.,]/g,''))).length;score+=common*5;if(words.length&&common===words.length)score+=8;if(g.length>45)score-=8;return score
}
function chooseGloss(values:string[],rank:number){const unique=[...new Set(values.map(v=>primary(v)).filter(Boolean))];const ranked=unique.map(v=>({v,score:glossScore(v)})).sort((a,b)=>b.score-a.score||a.v.length-b.v.length);const best=ranked[0];const threshold=rank<=800?8:4;return best&&best.score>=threshold?best.v:''}
...
export async function loadSpanishMap(words:CorpusWord[]):Promise<Record<string,string>>{
  ...
  const result:Record<string,string>={};for(const x of words){const key=x.de.toLowerCase();for(const c of candidates(x.de)){if(STANDARD_ES[c]){result[key]=STANDARD_ES[c];break}}}
  const questionable:string[]=[];const missing:string[]=[];
  try{
    const text=await fetch(DICT_URL).then(r=>{if(!r.ok)throw new Error(String(r.status));return r.text()});const dict:Record<string,string[]>={};
    for(const line of text.split('\n')){if(!line||line.startsWith('#')||!line.includes('::'))continue;const[esRaw,deRaw]=line.split('::',2);const options=glossParts(esRaw);if(!options.length)continue;for(const piece of deRaw.split(';')){const de=clean(piece).replace(/^sich\s+/i,'').toLowerCase();if(de&&wanted.has(de)){if(!dict[de])dict[de]=[];dict[de].push(...options)}}}
    for(const x of words){const key=x.de.toLowerCase();if(result[key])continue;let selected='';for(const c of candidates(x.de)){const options=dict[c]||[];if(options.length){selected=chooseGloss(options,x.rank);if(selected)break}}if(selected)result[key]=selected;else missing.push(x.de)}
    ...
    if(questionable.length)console.warn('Traducciones excluidas por baja confianza:',questionable);if(missing.length)console.info('Palabras sin traducción suficientemente confiable:',missing.length)
