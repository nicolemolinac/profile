...
import w1 from'./examWriting1';import w2 from'./examWriting2';
...
const bank=listening.filter(x=>x.text&&x.text.trim().length>35).map((x,i)=>{const k=keys(x.text);const right=k.length?k.join(' · '):'idea principal del audio';const d1=distr[(i+2)%distr.length].join(' · '),d2=distr[(i+5)%distr.length].join(' · ');const pos=i%3;const opts=[d1,d2];opts.splice(pos,0,right);return[`Corpus Listening · ${i+1}/${listening.length}`,x.text,'¿Qué grupo de palabras/ideas pertenece realmente al audio?',opts,pos,k.length?k:['escuchar','reconocer','detalle']];});
