...
function scores(p:Progress){const known=(p.known||[]).length,vocab=clamp(known/VOCAB_TARGET*100),done=p.listenDone||0,correct=p.listenCorrect||0,accuracy=done?correct/done:0,sample=Math.min(1,done/20),listening=clamp(accuracy*sample*100),speaking=clamp((p.speakDone||0)/15*100),writing=clamp((p.writeDone||0)/12*100),readiness=clamp(vocab*.15+listening*.4+speaking*.25+writing*.2);return{vocab,listening,speaking,writing,readiness,done}}
let patching=false;
function patchHome(){if(patching)return;const stats=[...document.querySelectorAll('.stats .stat')] as HTMLElement[];if(stats.length<4)return;patching=true;const p=readProgress(),s=scores(p);const vals:[string,string,number|null,string][]=[['Listening',s.done?`${s.listening}%`:'Sin medir',s.done?s.listening:null,s.done?`${s.done} respuestas registradas`:'Responde ejercicios para medirlo'],['Speaking',(p.speakDone||0)?`${s.speaking}%`:'Sin medir',(p.speakDone||0)?s.speaking:null,'Basado en prácticas completadas'],['Writing',(p.writeDone||0)?`${s.writing}%`:'Sin medir',(p.writeDone||0)?s.writing:null,'Basado en bloques practicados'],['Vocab',`${s.vocab}%`,s.vocab,`${(p.known||[]).length}/${VOCAB_TARGET} palabras aprendidas`]];stats.slice(0,4).forEach((el,i)=>{const[label,text,pct,note]=vals[i],bar=el.querySelector('.bar i') as HTMLElement|null;setText(el.querySelector('span'),label);setText(el.querySelector('b'),text);if(bar)bar.style.width=(pct??0)+'%';el.title=note});setText(document.querySelector('.orb strong'),s.readiness+'%');setText(document.querySelector('.orb span'),'progreso medido');patching=false}
