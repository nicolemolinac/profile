...
export function cheer(message?:string,kind:'good'|'try'|'neutral'='neutral'){
  ...
  if(span)span.textContent=lastMessage;
...
function patchHome(){
  ...
  patching=true;const s=scores(readProgress());
  const values:[string,string,number|null,string][]=[
    ...
    ['Speaking',(readProgress().speakDone||0)?`${s.speaking}%`:'Sin medir',(readProgress().speakDone||0)?s.speaking:null,'Basado en prácticas completadas'],
    ['Writing',(readProgress().writeDone||0)?`${s.writing}%`:'Sin medir',(readProgress().writeDone||0)?s.writing:null,'Basado en bloques practicados'],
    ['Vocab',`${s.vocab}%`,s.vocab,`${(readProgress().known||[]).length}/300 palabras objetivo`]
  ];
  stats.slice(0,4).forEach((el,i)=>{const [label,text,pct,note]=values[i];const span=el.querySelector('span');const b=el.querySelector('b');const bar=el.querySelector('.bar i') as HTMLElement|null;if(span)span.textContent=label;if(b)b.textContent=text;if(bar)bar.style.width=(pct??0)+'%';el.title=note});
  const orb=document.querySelector('.orb strong');if(orb)orb.textContent=s.readiness+'%';const orbLabel=document.querySelector('.orb span');if(orbLabel)orbLabel.textContent='progreso medido';
  patching=false;
}
...
function classifyClick(target:Element){
  ...
  if(txt.includes('Sí, fácil')||txt.includes('Ya lo sé')){setTimeout(()=>cheer(undefined,'good'),30);setTimeout(patchHome,80);return}
  if(txt.includes('Todavía no')){setTimeout(()=>cheer(undefined,'try'),30);setTimeout(patchHome,80);return}
  if(btn.closest('.options')){setTimeout(()=>{if(btn.classList.contains('right'))cheer(undefined,'good');else if(btn.classList.contains('wrong'))cheer(undefined,'try');patchHome()},40);return}
  if(txt.includes('Responder')){setTimeout(()=>cheer(pick(SPEAK),'good'),900);setTimeout(patchHome,1200)}
}

function start(){
  ensureMascot();patchHome();
  document.addEventListener('click',e=>classifyClick(e.target as Element),true);
  const observer=new MutationObserver(()=>patchHome());observer.observe(document.body,{childList:true,subtree:true});
