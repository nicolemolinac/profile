...
function suspicious(de:string,es:string){const d=key(de),s=clean(es).toLocaleLowerCase('es-ES');return !s||s===d||s.length>100||/https?:|<[^>]+>|\b(jota|ápice|miaja|adarme|maravedí|ochavo|ardite)\b/i.test(s)}
...
async function translateBrowser(de:string){try{const T=(globalThis as any).Translator;if(!T)return'';const availability=await T.availability?.({sourceLanguage:'de',targetLanguage:'es'});if(availability==='unavailable')return'';const tr=await T.create({sourceLanguage:'de',targetLanguage:'es'});const x=clean(await tr.translate(de));tr.destroy?.();return suspicious(de,x)?'':x}catch{return''}}
