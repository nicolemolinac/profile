...
import{TELC_COVERAGE_ES}from'./telcCoverageBoost';
...
async function worker(queue:string[],cache:Record<string,string>){while(queue.length){const de=queue.shift()!,k=key(de);if(cache[k])continue;try{const es=await translateOne(de);if(!suspicious(de,es))cache[k]=es}catch{}}}

export async function completeSpanishTranslations(words:{de:string}[],existing:Record<string,string>){
 const staticChecked={...TELC_COVERAGE_ES,...EXAM_BOOST_ES};
 const out:Record<string,string>={...existing,...staticChecked};
 let official=0,common=0,existingKept=0,fallback=0;
 for(const w of words){const k=key(w.de),telc=officialTelcSpanish(w.de),commonGloss=COMMON_B1[k]||COMMON_B1[k.replace(/^sich\s+/,'')];if(telc){out[k]=telc;official++;continue}if(staticChecked[k]){out[k]=staticChecked[k];continue}if(commonGloss){out[k]=commonGloss;common++;continue}if(out[k]&&!suspicious(w.de,out[k]))existingKept++;else delete out[k]}
 Object.assign(out,staticChecked);
 const cache=read(FALLBACK_CACHE);let queue=words.filter(w=>!out[key(w.de)]&&!cache[key(w.de)]).map(w=>w.de);
 for(let pass=0;pass<2&&queue.length;pass++){const pending=[...queue];await Promise.all(Array.from({length:Math.min(4,pending.length)},()=>worker(pending,cache)));queue=queue.filter(de=>!cache[key(de)])}
 save(FALLBACK_CACHE,cache);for(const w of words){const k=key(w.de);if(!out[k]&&cache[k]&&!suspicious(w.de,cache[k])){out[k]=cache[k];fallback++}}
 Object.assign(out,staticChecked);
 const missing=words.filter(w=>!out[key(w.de)]).map(w=>w.de);save(AUDIT_KEY,{checked:words.length,translated:words.length-missing.length,missing,official,common,existingKept,fallback,generatedAt:new Date().toISOString(),reference:'official TELC B1 + curated exam corpus + TELC coverage safety net'});save(CORPUS_CACHE,out);return out
}

export function getTranslationQualityAudit(){try{return JSON.parse(localStorage.getItem(AUDIT_KEY)||'null')}catch{return null}}
export async function bootstrapCompleteTranslations(){const words=await loadCorpusWords();const current={...read(CORPUS_CACHE),...TELC_COVERAGE_ES,...EXAM_BOOST_ES};const currentMissing=words.filter(w=>!current[key(w.de)]).length;if(currentMissing===0){save(CORPUS_CACHE,current);save(AUDIT_KEY,{checked:words.length,translated:words.length,missing:[],generatedAt:new Date().toISOString(),reference:'cached complete corpus + official TELC coverage + curated exam corpus'});return current}const base=Object.keys(current).length?current:await loadSpanishMap(words);return await completeSpanishTranslations(words,base)}
